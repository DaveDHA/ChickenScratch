module ChickenScratch.HtmlExpressions

type ScratchNodeBuilderNode =
| ScratchNodeAttribute of string * string
| ScratchNodeChild of ScratchNode

module ScratchNodeBuilderNode =
    let MergeAttributes items =
        items
        |> Seq.choose (function | ScratchNodeAttribute (k,v) -> Some (k,v) | _ -> None)
        |> Seq.groupBy fst
        |> Seq.map (fun (key, items) -> (key, items |> Seq.map snd |> Seq.distinct |> String.concat " "))
        |> Map.ofSeq

    let CollectChildren items = items |> List.choose (function | ScratchNodeChild t -> Some t | _ -> None)
...
type ScratchNodeBuilder(tag, allowChildren) = 
    ...
    member this.Yield (x : ScratchElement) = 
        x |> Element |> ScratchNodeChild |> this.Yield

    member this.YieldFrom (xs : ScratchElement seq) = 
        xs |> Seq.map (Element >> ScratchNodeChild) |> this.YieldFrom

    member this.Yield (x : ScratchNode) = ScratchNodeChild x |> this.Yield

    member this.YieldFrom (xs : ScratchNode seq) = xs |> Seq.map ScratchNodeChild |> this.YieldFrom

    member this.Yield (x : string) = x |> EncodedContent |> this.Yield

    member this.YieldFrom (xs : string seq) = xs |> Seq.map EncodedContent |> this.YieldFrom

    member this.Yield<'t when 't :> IScratchNodeSource> (x : 't) = 
        (x :> IScratchNodeSource).GetScratchNodes() |> this.YieldFrom
...
[<AutoOpen>]
module Tags = 
    let tag t = scratchNode t true
    let selfClosingTag t = scratchNode t false
...
[<AutoOpen>]
module Attributes = 
    let attr k v = ScratchNodeAttribute (k,v)
